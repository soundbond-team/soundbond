...
exports.bestTags = async (req, res) => {
  try{
    //!AJOUTER DES COMMENTAIRES
    const favs = await db.sequelize.query("SELECT t.tag, count(tp.tag) as apparition FROM Tags t, Tag_Post tp, Posts p\
    WHERE t.id=tp.tagging_id AND tp.post_id=p.id AND strftime('%m',p.createdAt)=strftime('%m',DATE())",
    {
      type: QueryTypes.SELECT
    })
    res.json(favs);
